...
# Fail2Ban 自动安装配置脚本 (Debian/Ubuntu专用�?
...
install_fail2ban() {
    ...
    # 检查安装是否成�?    if ! command -v fail2ban-server &> /dev/null; then
        log_error "Fail2Ban 安装失败"
        exit 1
    fi
    
    log_info "Fail2Ban �?rsyslog 安装成功"
}

# 配置Nginx过滤�?configure_nginx_filters() {
    print_header
    echo -e "${CYAN}${BOLD}配置Nginx过滤�?{RESET}"
    print_separator
...
EOF
    ...
    # 2. 防CC攻击过滤�?    cat > /etc/fail2ban/filter.d/nginx-cc.conf << 'EOF'
[Definition]
...
uninstall_fail2ban() {
    ...
    echo -e "${YELLOW}警告：此操作将完全卸载Fail2Ban并删除所有配�?{NC}"
    echo ""
    echo "将要执行的操作："
    echo "1. 停止Fail2Ban服务"
    echo "2. 禁用开机启�?
    ...
    # 2. 禁用开机启�?    echo -e "${GREEN}[2/6] 禁用开机启�?..${NC}"
    systemctl disable fail2ban 2>/dev/null || true
    
    # 3. 卸载软件�?    echo -e "${GREEN}[3/6] 卸载Fail2Ban软件�?..${NC}"
    apt-get remove --purge -y fail2ban
    
    # 4. 清理依赖�?    echo -e "${GREEN}[4/6] 清理无用依赖�?..${NC}"
    apt-get autoremove -y
    
    # 5. 清理包缓�?    echo -e "${GREEN}[5/6] 清理包缓�?..${NC}"
    apt-get clean
    
    # 6. 删除配置文件和日�?    echo -e "${GREEN}[6/6] 删除配置文件和日�?..${NC}"
    
    # 删除主配置文�?    rm -f /etc/fail2ban/jail.local
    ...
    # 删除锁文�?    rm -f /var/run/fail2ban/fail2ban.sock 2>/dev/null || true
    ...
    print_separator
    echo -e "${GREEN}${BOLD}Fail2Ban 已完全卸�?{NC}"
    echo ""
    echo -e "${YELLOW}建议重启系统以确保所有更改生�?{NC}"
    echo ""
...
edit_jail_config() {
    ...
    echo -e "${YELLOW}提示�?{NC}"
    echo "1. 可以修改各监狱的 enabled 状�?(true/false)"
...
# 查看封禁状�?view_status() {
    while true; do
        print_header
        echo -e "${CYAN}${BOLD}查看封禁状�?{RESET}"
        print_separator
        
        echo -e "${MAGENTA}${BOLD}请选择要查看的监狱�?{RESET}"
        echo ""
        echo -e "${GREEN}[1]${NC} 查看所有监狱状�?
        echo -e "${GREEN}[2]${NC} 查看SSH监狱状�?
        echo -e "${GREEN}[3]${NC} 查看Nginx恶意扫描监狱状�?
        echo -e "${GREEN}[4]${NC} 查看Nginx CC攻击监狱状�?
        ...
        case $choice in
            1)
                echo ""
                echo -e "${CYAN}════════════ 所有监狱状�?════════════${NC}"
                fail2ban-client status
                ;;
            2)
                echo ""
                echo -e "${CYAN}════════════ SSH监狱状�?════════════${NC}"
                fail2ban-client status sshd
                ;;
            3)
                echo ""
                echo -e "${CYAN}════════════ Nginx恶意扫描监狱状�?════════════${NC}"
                fail2ban-client status nginx-bad-request
                ;;
            4)
                echo ""
                echo -e "${CYAN}════════════ Nginx CC攻击监狱状�?════════════${NC}"
    ...
    while true; do
        ...
        case $choice in
            ...
            7)
                ...
                print_separator
                echo -e "${YELLOW}最�?0条日志：${NC}"
                journalctl -u fail2ban -n 20 --no-pager
                press_any_key
                ;;
            8)
                print_header
                echo -e "${CYAN}${BOLD}查看Fail2Ban运行状�?{RESET}"
                print_separator
