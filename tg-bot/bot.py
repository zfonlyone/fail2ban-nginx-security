...
FAIL2BAN_RESTART_SUPPRESS_MINUTES = int(os.environ.get("FAIL2BAN_RESTART_SUPPRESS_MINUTES", "10"))
...
def fail2ban_started_recently() -> bool:
    """fail2ban 刚重启后的短窗口内，抑制高频封禁误报。"""
    try:
        out = run_host_cmd("systemctl show fail2ban -p ActiveEnterTimestampMonotonic --value 2>/dev/null")
        started_us = int(str(out).strip().splitlines()[-1])
        if started_us <= 0:
            return False
        uptime_seconds = time.monotonic() - (started_us / 1_000_000)
        return uptime_seconds >= 0 and uptime_seconds < FAIL2BAN_RESTART_SUPPRESS_MINUTES * 60
    except Exception:
        return False
...
async def scheduled_anomaly_check(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. 检测每分钟封禁数量 (高频攻击检测)
    try:
        if not fail2ban_started_recently():
            auto_bans = read_ban_json("auto-banned.json")
            now_ts = datetime.now(timezone.utc)
