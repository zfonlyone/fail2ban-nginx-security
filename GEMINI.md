...
- 运行目录：`/etc/security-guard`
  - 只保留运行所需的 `docker-compose.yml`、`.env`、`ban/`、`state/`。
...
3. 同步运行期 `docker-compose.yml` 到 `/etc/security-guard`
4. 把 healthcheck 与运维脚本安装到 `/usr/local/bin`
5. 清理运行目录残留的 `scripts/`、`tg-bot/` 源码
6. 在 `/etc/security-guard` 启动或更新容器
